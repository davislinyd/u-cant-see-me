...
On Gmail, the popup offers a local **Protect this email** action. It derives only rendered thread/message IDs and can mask the conversation subject, message bodies, collapsed previews, sender/recipient metadata, and the matching inbox/search row's sender, subject, and snippet. In split-pane inboxes Gmail can keep the URL unchanged while the selected email changes; protection is therefore bound to the rendered thread ID on the current reading-pane subject, so switching to another email does not mask that email's body. A rule can additionally include a message ID, so sibling message bodies remain visible. Gmail selectors, route parsing, resolvers, and the Gmail-only fail-closed guard are isolated under `src/adapters/gmail/`. If a protected Gmail route cannot be structurally verified, the guard remains visible with Retry and a runtime-only temporary-reveal option; it never silently falls back to plaintext.

The Options page supports safe rule filtering, enable/disable, style and scope editing, duplication, deletion, current-page locator testing, and JSON import/export. Exports include only settings and rule metadata; imports are schema-validated as data and never execute selectors or configuration. The popup can temporarily reveal, open edit mode, disable rules for the current page/site, or remove page rules. Its single **Manage masks & settings** button opens the Options page for both rule and general settings management. Chromium commands use Alt+Shift+S (selection), Alt+Shift+V (temporary reveal), and Alt+Shift+L (remask); holding Alt+Shift+H reveals only while held. Command shortcuts can be changed in Chromium's extension shortcut UI.

The popup and Options page ship in English and Traditional Chinese (Taiwan). A language segmented control on both surfaces persists the choice in synced settings and re-renders immediately; product terms such as Gmail, Strict Mask, Blur, URL, and locator stay in English in both languages. The language preference travels with configuration import/export.
